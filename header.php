...
<head>
    ...
    <?php wp_head(); ?>
    <script>
        // Appliquer le mode de couleur immédiatement dans le header
        (function() {
            const savedMode = localStorage.getItem('toggle-mode');
            if (savedMode === 'true') {
                document.documentElement.classList.add('mode-orange');
                document.documentElement.classList.remove('mode-violet');
            } else {
                document.documentElement.classList.add('mode-violet');
                document.documentElement.classList.remove('mode-orange');
            }
        })();
    </script>
</head>
<body <?php body_class(); ?>>
        ...
        <nav class="navbar navbar-expand-lg navbar-dark">
            <div class="container-fluid">
                ...
                <a class="navbar-brand" href="<?php echo esc_url(home_url('/')); ?>">
                    <img src="<?php echo esc_url(get_template_directory_uri()); ?>/Images/Logo_BeBeats_mobile.svg" alt="BeBeats Logo" class="logo-img" />
                </a>
    ...
    <nav class="main-nav" aria-label="Navigation principale">
        ...
        $is_home = is_front_page() || is_page('feed-page');
        $is_recherche = is_page('recherche') || is_page('resultats') || is_search();
        $is_contribuer = is_page('contribuer');
        $is_event = is_page('event');
        $is_reglages = is_page('reglages');
        $is_auth = is_page('auth-start') || is_page('connexion') || is_page('inscription');
        ?>
        
        <a href="<?php echo esc_url(home_url('/')); ?>" class="nav-item <?php echo $is_home ? 'active' : ''; ?>" aria-label="Accueil">
            <div class="nav-icon">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <a href="<?php echo esc_url(home_url('/event')); ?>" class="nav-item <?php echo $is_event ? 'active' : ''; ?>" aria-label="Event">
        ...
        <a href="<?php echo esc_url(home_url('/reglages')); ?>" class="nav-item nav-item-settings <?php echo $is_reglages ? 'active' : ''; ?>" aria-label="Réglages">
            <div class="nav-icon">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
        ...
        <?php if (is_user_logged_in()): 
            $current_user_bottom = wp_get_current_user();
            $profile_photo_bottom = get_user_meta($current_user_bottom->ID, 'bebeats_profile_photo', true);
            if (empty($profile_photo_bottom)) {
                $profile_photo_bottom = get_avatar_url($current_user_bottom->ID, array('size' => 64));
            }
        ?>
            <a href="<?php echo esc_url(home_url('/profil')); ?>" class="nav-item nav-item-login <?php echo is_page('profil') ? 'active' : ''; ?>" aria-label="Profil">
                <div class="nav-icon nav-icon-profile">
                    <img src="<?php echo esc_url($profile_photo_bottom); ?>" alt="<?php echo esc_attr($current_user_bottom->display_name); ?>" class="nav-profile-img">
                </div>
                <span class="nav-label">Profil</span>
            </a>
        <?php else: ?>
            <a href="<?php echo esc_url(home_url('/auth-start')); ?>" class="nav-item nav-item-login <?php echo $is_auth ? 'active' : ''; ?>" aria-label="Connexion">
                <div class="nav-icon">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
